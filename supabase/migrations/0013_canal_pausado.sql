...
-- disparando. Agora a coluna vale.
--
-- Aqui fica a metade das campanhas. A outra metade — o bot não responder — é
-- no webhook, em app/src/app/api/webhooks/evolution/route.ts.

CREATE OR REPLACE FUNCTION chat.claim_next_send()
 RETURNS TABLE(recipient_id uuid, campaign_id uuid, channel_id uuid, wa_id text, name text, media_kind chat.campaign_media, body text, media_url text, media_filename text, media_mime text)
 LANGUAGE plpgsql
 SECURITY DEFINER
